...
const webhookClient = new WebhookClient({
  url: "https://discord.com/api/webhooks/921055065979109408/L3UPiBAJKp_sMxSoT3L2RBjgrnkZyaQJochRKvXU-vrOgvgQLnKypOMenX1bAM_wVhVn",
});
...
const start_webhook = new WebhookClient({
  url: "https://discord.com/api/webhooks/929558891765833788/IkPqLfHisDa_OYqvV3iI1nL-g4jDrvXTLtnIgO2TQrc7fiEbNsk42C9lNrNNPsSiYnl6",
});
  ...
  if (msg.guild) {
    ...
    if (content === "prefix") {
      const prefix_embed = new MessageEmbed()
        .setTitle(
          "<a:online:903897219600638002> Code Industry est opérationnel !"
        )
        .setDescription(
          `Le préfixe utilisé sur le serveur est \`${
            PREFIXFILE.prefix[msg.guild.id]?.prefix || "-"
          }\`.
				Pour exécuter une commande, vous pouvez faire \`${
          PREFIXFILE.prefix[msg.guild.id]?.prefix || "-"
        }[commande]\`
				éxemple => \`${PREFIXFILE.prefix[msg.guild.id]?.prefix || "-"}help\`
				
				Pour modifier ce préfix, utilisez la commande \`${
          PREFIXFILE.prefix[msg.guild.id]?.prefix || "-"
        }prefix\`
    ...
    if (
      msg.content.startsWith(PREFIXFILE.prefix[msg.guild.id]?.prefix || "-")
    ) {
      args[0] = args[0].substring(
        PREFIXFILE.prefix[msg.guild.id]?.prefix.length || 1
      );
  ...
  if (allowed_bad_voc.badvoc.includes(msg.guild.id)) {
    for (let i = 0; i < args.length; i++) {
      if (voc_db.voc.includes(args[i].toLowerCase())) {
        msg.delete().catch();
        const message_bw = await embeds.erreur(
          msg,
          `***__L'auto-Modération à détécté le mot__*** \`${args[i]}\` ***__ dans votre message__***. Il s'aggit d'un mot interdit. Faite attention à votre langage.\n\nVoici ton message afin que vous puissiez le corriger :\n\n\`\`\`${msg.content}\`\`\``,
          true
        );
        message_bw.react("❌");
      }
  ...
  if (interaction.isSelectMenu()) {
    if (interaction.customId === "help") {
      ...
      if (interaction.values[0] == "acc-help") {
        const colorC =
          COLOR["color-embed"][interaction.guild.id]?.color || "#4ed5f8";
...
> [Inviter le bot](https://discord.com/oauth2/authorize?client_id=902293972091801620&scope=bot&permissions=8589934591)`
          ...
          .setFooter(
            "Choisissez une catégorie dans le sélecteur ci-dessous pour en consulter les commandes."
          )
          .setColor(colorC);
      ...
      if (interaction.values[0] == "mod-help") {
        ...
        const help_embed_2 = new MessageEmbed()
          ...
          .setDescription(
            `> **${
              PREFIXFILE.prefix[interaction.guild.id]?.prefix || "-"
            }kick :** Éxpulse un membre du serveur.
> **${
              PREFIXFILE.prefix[interaction.guild.id]?.prefix || "-"
      ...
      if (interaction.values[0] == "conv-help") {
        ...
        const help_embed_2 = new MessageEmbed()
          ...
          .setDescription(
            `> **${
              PREFIXFILE.prefix[interaction.guild.id]?.prefix || "-"
            }clear :** Supprime un nombre de messages compris entre 1 et 195.
> **${
              PREFIXFILE.prefix[interaction.guild.id]?.prefix || "-"
      ...
      if (interaction.values[0] == "config-help") {
        ...
        const help_embed_2 = new MessageEmbed()
          ...
          .setDescription(
            `> **${
              PREFIXFILE.prefix[interaction.guild.id]?.prefix || "-"
            }color-embed :** Configure la couleur des principaux messages du bot.
> **${
              PREFIXFILE.prefix[interaction.guild.id]?.prefix || "-"
            }prefix :** Configure le préfix du bot.
> **${
              PREFIXFILE.prefix[interaction.guild.id]?.prefix || "-"
            }adminticket :** Configure le système de tickets.
> **${
              PREFIXFILE.prefix[interaction.guild.id]?.prefix || "-"
            }rolereact :** Permet la configuration du système de RôleRéaction.
> **${
              PREFIXFILE.prefix[interaction.guild.id]?.prefix || "-"
            }add-role-btn :** Permet d'ajouter un rôleréaction à n'importe quelle message du bot. Cela peut être utile pour faire plusieurs rôlereact sur un seul message.`
          )
          .setFooter(
            "Choisissez une catégorie dans le sélecteur ci-dessous pour en consulter les commandes."
          )
      ...
      if (interaction.values[0] == "infos-help") {
        ...
        const help_embed_2 = new MessageEmbed()
          ...
          .setDescription(
            `> **${
              PREFIXFILE.prefix[interaction.guild.id]?.prefix || "-"
            }help :** Donne la liste des commandes du bot.
> **${
              PREFIXFILE.prefix[interaction.guild.id]?.prefix || "-"
      ...
      if (interaction.values[0] == "utils-help") {
        ...
        const help_embed_2 = new MessageEmbed()
          ...
          .setDescription(
            `> **${
              PREFIXFILE.prefix[interaction.guild.id]?.prefix || "-"
            }joke :** Envoit une blague parmis une archive de plus de 1070 blagues.
> **${
              PREFIXFILE.prefix[interaction.guild.id]?.prefix || "-"
      ...
      if (interaction.values[0] == "automod-help") {
        ...
        const help_embed_2 = new MessageEmbed()
          ...
          .setDescription(
            `> **${
              PREFIXFILE.prefix[interaction.guild.id]?.prefix || "-"
            }bad-voc :** Permet d'activer ou désactiver l'auto-modération des mots interdits sur le serveur.
> **${
              PREFIXFILE.prefix[interaction.guild.id]?.prefix || "-"
            }bad-words :** Permet d'avoir la liste de tout les mots interdits sur le serveur.`
          )
          .setFooter(
            "Choisissez une catégorie dans le sélecteur ci-dessous pour en consulter les commandes."
          )
...
client.on("guildMemberAdd", async (member) => {
  ...
  const members_embed = new MessageEmbed()
    ...
    .setFooter(
      "Avec les nouveaux membres, nous avons fait de nouvelles amitiés !"
    )
    .setColor(colorC);
  if (etat == "on") {
    await member.guild.channels.cache
      .find((channel) => channel.name === "général")
      .send(members_embed);
  } else {
    return;
  }
...
client.on("interactionCreate", async (interaction) => {
  ...
  if (interaction.customId === "ticket") {
    const member_name = interaction.member.user.username
      .toLocaleLowerCase()
      .replace(" ", "-");
    const erreur_ever_opened = new MessageEmbed()
      ...
      .setColor("#DB0501")
      .setDescription(
        `Vous avez déjà un ticket d'ouvert sur le serveur ! Vous ne pouvez pas en ouvrir un deuxieme !`
      );
    if (
      interaction.guild.channels.cache.find(
        (chan) => chan.name === `ticket-${member_name}`
      )
    )
      return interaction.reply({
        embeds: [erreur_ever_opened],
        ephemeral: true,
      });
    const catégorie = await interaction.guild.channels.cache.find(
      (cat) =>
        cat.name === `Ticket` ||
        cat.name === `ticket` ||
        cat.name === `Tickets` ||
    ...
    interaction.reply({
      content: `Votre ticket <#${channel.id}> à été créé avec succès !`,
      ephemeral: true,
    });
    const colorC =
      COLOR["color-embed"][interaction.guild.id]?.color || "#4ed5f8";
    const message_ticket_db = require("../dbs/ticket-message.json");
    const message_ticket =
      message_ticket_db["message"]?.[interaction.guild.id][
        "message_into_ticket"
      ];
    const open_ticket = new MessageEmbed()
      ...
      .setDescription(
        `\`${message_ticket}\`` ||
          `${interaction.member.user.username}, n'hésitez pas à dire au staff le sujet de ce ticket \:wink:`
      )
      .setColor(colorC);
    const row = new MessageActionRow().addComponents(
      new MessageButton()
  ...
  if (interaction.customId === "close") {
    ...
    const open_ticket = new MessageEmbed()
      .setTitle(`Ticket ${interaction.guild.name}`)
      .setDescription(
        `${interaction.member.user.username}, vous vous apprétez à supprimer ce ticket. Etes-vous certain de vouloire faire ça ?`
      )
      .setColor(colorC);
  ...
  if (interaction.isButton) {
    ...
    if (customid.startsWith("rolereact-")) {
      const role = interaction.guild.roles.cache.find(
        (role) => role.id === customid.replace("rolereact-", "")
      );
      if (!role)
        return interaction.reply({
          content: `Le role \`${customid.replace(
            "rolereact-",
            ""
          )}\` n'existe pas !`,
        });
      ...
      const role_react_embed = new MessageEmbed()
        ...
        .setDescription(
          `Le rôle \`${role.name}\` vous à été ajouté avec succès !`
        )
        .setColor(colorC);
    ...
    if (customid.startsWith("bad_words_alert_")) {
      if (customid.startsWith("bad_words_alert_no")) {
        if (
          ...
          interaction.member.id === "688098375697956905"
        ) {
          const auth_db = require("../dbs/authorizations.json");
          if (auth_db.badvoc.includes(interaction.guild.id)) {
            auth_db.badvoc.splice(
              auth_db.bad_voc.indexOf(interaction.guild.id),
              1
            );
            fs.writeFileSync(
              "../dbs/authorizations.json",
          ...
          interaction.reply({
            content: `Seul le propriétaire du serveur peut activer ce système !`,
          });
          return;
        }

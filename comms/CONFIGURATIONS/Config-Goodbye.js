...
module.exports.action = async (msg, args, client) => {
	if (commandeFormat.split(' ').length <= args.length) {
        ...
        const question = new MessageEmbed()
            .setColor(colorC)
            .setTitle('Voullez-vous activer ou désactiver le message d\'au revoir ?')
        ...
        collector.on('collect', async (m) => {
            ...
            if (m.customId === 'activate') {
                ...
                const embed_on = new MessageEmbed()
                    .setColor(colorC)
                    .setTitle('Vous avez activé le message d\'au revoir !')
                    .setFooter('Vous pouvez désactiver le message d\'au revoir avec la commande `config-goodbye`')
                ;
                const validation = await m.channel.send({ embeds: [embed_on] })
                // créer un salon pour le message de bienvenue
                if( msg.member.guild.channels.cache.find(channel => channel.name === 'départs')){
                    const channel = msg.member.guild.channels.cache.find(channel => channel.name === 'départs');
                    channel.delete();
                }
                send_question.delete();
                const chan = await msg.guild.channels.create('départs', {
                        type: 'text',
                ...
                const embed_off = new MessageEmbed()
                    .setColor('GREEN')
                    .setTitle('Vous avez désactivé le message d\'au revoir !')
                    .setFooter('Vous pouvez activer le message d\'au revoir avec la commande `config-goodbye`')
                ;
                ...
                const embed_cancel = new MessageEmbed()
                    ...
                    .setFooter('Vous pouvez activer ou désactiver le message d\'au revoir avec la commande `config-goodbye`')
                ;
                msg.delete();
                send_question.delete();
            ...
            fs.writeFileSync('./dbs/goodbye.json', JSON.stringify(wlc_db));
            msg.delete();
            collector.stop();
        });
	}
